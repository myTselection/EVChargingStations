...
def create_schema(entry, option=False):
    ...
    fuel_types = [
        FuelType.ELECTRIC_T1.name_lowercase,
        FuelType.ELECTRIC_T2.name_lowercase,
        FuelType.ELECTRIC_DC.name_lowercase
    ]
    ...
    data_schema[vol.Required("fuel_type", default=[], description="Fuel Type")] = SelectSelector(
                        SelectSelectorConfig(
                            multiple=True,  # Enables multi-select
                            options=[{"value": opt, "label": opt} for opt in fuel_types]
                        )
                    )
    data_schema[
